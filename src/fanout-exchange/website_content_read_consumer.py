...
from constant import DEFAULT_RABBITMQ_URL, \
    WEBSITE_CONTENT_READ_QUEUE_NAME, \
    WEBSITE_EXCHANGE_NAME, \
    WEBSITE_CONTENT_READ_ROUTINGKEY
...
class RabbitMQWebsiteNameMessageConsumer:
    def __init__(self):
        ...
        try:
            ...
            channel.queue_declare(queue=WEBSITE_CONTENT_READ_QUEUE_NAME)

            # Bind the queue with exchange using the routing key
            channel.queue_bind(queue=WEBSITE_CONTENT_READ_QUEUE_NAME,
                               exchange=WEBSITE_EXCHANGE_NAME,
                               routing_key=WEBSITE_CONTENT_READ_ROUTINGKEY)
    ...
    def consume(self):
        ...
        channel.basic_consume(self.process_message,
                              queue=WEBSITE_CONTENT_READ_QUEUE_NAME)
        channel.start_consuming()


    def process_message(self, channel, method, properties, website_name):
        print 'Received website name to read the content:', website_name
